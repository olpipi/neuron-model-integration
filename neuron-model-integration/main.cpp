...
#define TIME 100        //Время моделирования

#define NUMAVE 40000    //Количество прогонов для усреднения
double FREQ = 100;        //Частота синусоидального сигнала
...
void escape_time_parallel_launch(ofstream *fout){
    ...
    while (dispercy < 50) {
        ...
        EscTime2 = 0.0;
        system("cls");
        std::cout << SIGNAL<< "   " << FREQ << ":  " << dispercy;
        //unsigned int start_time = clock();
        setParams(STEP, FREQ);
        double noiseParam = sqrt(2 * dispercy * STEP);
        double noise = 0.0;

#pragma omp parallel reduction(+:EscTime,EscTime2) num_threads(8) private(noise)
        {
